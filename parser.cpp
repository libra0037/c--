...
void Parser::reduce(int rule)
{
	...
	case 2: // stmt_seq <- stmt_seq WHILE exp stmt_seq SEMI
		...
		gen = gens.size();
		gens.push_back([ids = chg_list, line2 = n2.line, gen1 = n1.gen, gen3 = n3.gen, gen4 = n4.gen](int &block, int *reg_list)->tails_attr
		{
			f[gen1](block, reg_list);
		...
		chg_list.insert(n1.chg_list.begin(), n1.chg_list.end());
		break;
...
int Parser::parse()
{
	...
	do
	{
		...
		for(int i = 0; ; i += 2)
		{
			...
			{ // shift
				stk.emplace(Action[top.state][i + 1], now_line, tk.val, std::set<int>());
				tk = scan();
				break;
			}
